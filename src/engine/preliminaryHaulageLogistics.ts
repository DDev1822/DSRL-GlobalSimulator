...
export function createPreliminaryHaulageInputs(
  ...
  for (const id of ROUTE_IDS) routes[id] = { ...defaults[id], ...(overrides.routes?.[id] ?? {}), id, assumptionBasis: 'dsrl-scenario' };
  return {
...
export function validatePreliminaryHaulageInputs(inputs: PreliminaryHaulageInputs): string[] {
  ...
  for (const id of ROUTE_IDS) {
    ...
    for (const [label, value] of [['velocidad cargado', item.loadedSpeedKph], ['velocidad vacío', item.emptySpeedKph], ['payload', item.payloadTonnes], ['horas operativas', item.operatingHoursPerPeriod]] as const) {
...
function evaluateRoute(
  ...
  const cycleTimeMinutes = loadedTravelTimeMinutes + emptyTravelTimeMinutes + definition.loadingMinutes + definition.dumpingMinutes + definition.spottingMinutes + definition.fixedDelayMinutes;
  ...
  const discountedMarginAfterHaulageUsdM = marginBeforeHaulageUsdM / Math.pow(1 + discountRate, period) - discountedLogisticsCostUsdM;
  ...
  return {
    ...
    bottleneck: capacityDeficitMt > EPS ? 'capacity-deficit' : cycleTimeMinutes > 60 ? 'cycle-time' : fuelShare > 0.55 ? 'fuel-cost' : 'none',
  };
}

function periodMasses(report: IntegratedRouteEconomicReport, index: number): Record<HaulageRouteId, number> {
...
function periodMargins(report: IntegratedRouteEconomicReport, index: number): Record<HaulageRouteId, number> {
  const recovery = report.routeRecoveryReport.periods[index];
  ...
  return {
    'mill-direct': millFeed > EPS ? economics.routes.mill.operatingMarginUsdM * recovery.routes.mill.directFeedMassMt / millFeed : 0,
    'leach-direct': leachFeed > EPS ? economics.routes.leach.operatingMarginUsdM * recovery.routes.leach.directFeedMassMt / leachFeed : 0,
    dump: 0,
    'mill-stockpile': 0,
    'leach-stockpile': 0,
    'mill-reclaim': millFeed > EPS ? economics.routes.mill.operatingMarginUsdM * recovery.routes.mill.reclaimedMassMt / millFeed : 0,
    'leach-reclaim': leachFeed > EPS ? economics.routes.leach.operatingMarginUsdM * recovery.routes.leach.reclaimedMassMt / leachFeed : 0,
  };
}

function aggregateRoute(id: HaulageRouteId, periods: PreliminaryHaulageLogisticsReport['periods'], definition: PreliminaryHaulageRouteDefinition): PreliminaryHaulageRouteTotals {
  ...
  return cases.map((scenario) => {
    ...
    for (const period of report.periods) {
      for (const id of ROUTE_IDS) {
        ...
        const evaluated = evaluateRoute(routes[id], period.period, source.demandMassMt, source.marginBeforeHaulageUsdM, report.inputs.economics.discountRate);
...
export function buildPreliminaryHaulageLogistics(
  ...
  const economicReport = buildIntegratedRouteEconomics(dataset, phase, scope, benchHeightM, economic, gradeConfirmation, costBasis, inputs.economics);
  const periods = economicReport.periods.map((economicPeriod, index) => {
    ...
    for (const id of ROUTE_IDS) routes[id] = evaluateRoute(inputs.routes[id], economicPeriod.period, masses[id], margins[id], inputs.economics.discountRate);
    const primaryHaulageMassMt = routes['mill-direct'].demandMassMt + routes['leach-direct'].demandMassMt + routes.dump.demandMassMt + routes['mill-stockpile'].demandMassMt + routes['leach-stockpile'].demandMassMt;
    ...
    const marginAfterHaulageUsdM = marginBeforeHaulageUsdM - totalLogisticsCostUsdM;
    const discountedMarginAfterHaulageUsdM = ROUTE_IDS.reduce((sum, id) => sum + routes[id].discountedMarginAfterHaulageUsdM, 0);
  ...
  const totalDiscountedMarginAfterHaulageUsdM = periods.reduce((sum, period) => sum + period.discountedMarginAfterHaulageUsdM, 0);
  ...
  const expectedRehandleMassMt = economicReport.routeRecoveryReport.periods.reduce((sum, period) => sum + period.routes.mill.reclaimedMassMt + period.routes.leach.reclaimedMassMt, 0);
  ...
  const core: Omit<PreliminaryHaulageLogisticsReport, 'sensitivity'> = {
    ...
    reconciliation: {
      ...
      tripPayloadCloses: ROUTE_IDS.every((id) => close(routeTotals[id].requiredTrips * inputs.routes[id].payloadTonnes / 1_000_000, routeTotals[id].demandMassMt)),
      capacityDeficitReported: ROUTE_IDS.every((id) => routeTotals[id].demandMassMt <= routeTotals[id].capacityMassMt + routeTotals[id].capacityDeficitMt + EPS),
      ...
      costComponentsClose: close(totalLogisticsCostUsdM, totalFuelCostUsdM + totalMaintenanceCostUsdM + totalTireCostUsdM + totalOtherCostUsdM),
      unitCostCloses: totalDemandMassMt <= EPS || close((coreUnitCost(totalLogisticsCostUsdM, totalDemandMassMt)) * totalDemandMassMt, totalLogisticsCostUsdM),
      marginAfterHaulageCloses: close(totalMarginAfterHaulageUsdM, totalMarginBeforeHaulageUsdM - totalLogisticsCostUsdM),
      destinationIdentityPreserved: inputs.routes['mill-direct'].sourceDestination === 'Mill' && inputs.routes['leach-direct'].sourceDestination === 'Leach' && inputs.routes.dump.sourceDestination === '_DUMP_',
      unknownDestinationsReported: economicReport.reconciliation.unknownDestinationsReported,
      noImpossibleNegativeBalances: ROUTE_IDS.every((id) => [routeTotals[id].demandMassMt, routeTotals[id].capacityMassMt, routeTotals[id].requiredTrips, routeTotals[id].requiredTruckHours, routeTotals[id].fuelLiters, routeTotals[id].totalLogisticsCostUsdM].every((value) => value >= -EPS)),
    },
    methodology: {
      routePolicy: 'observed-destination-fixed-logistics-evaluation',
      observedDestinationField: 'NPVPDEST',
...
function coreUnitCost(costUsdM: number, massMt: number): number {
